...
function scr_load_chamber_types() {
    global.chamber_type_data = [];
    global.chamber_type_lookup = {};
    ...
    for (var i = 0; i < array_length(_files); i++) {
        ...
        for (var j = 0; j < array_length(_entries); j++) {
            ...
            var _idx = array_length(global.chamber_type_data);
            array_push(global.chamber_type_data, _entries[j]);
            global.chamber_type_lookup[_entries[j].type] = _idx;
        }
    }
}
...
function scr_get_chamber_type(_type_string) {
    if (!global.chamber_type_lookup.exists(_type_string)) return undefined;
    return global.chamber_type_data[global.chamber_type_lookup[_type_string]];
}
